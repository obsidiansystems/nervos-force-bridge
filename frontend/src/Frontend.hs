...
form :: (PostBuild t m, DomBuilder t m, Prerender t m) => Event t () -> Double -> BridgeDirection -> m (Dynamic t (Maybe Double))
...
recentTransactionsFeed newHash = mdo
  ...
  -- TODO(skylar): Make tick never fire when newHash fires, aka remember the exact function for that
...
txPrompt txHash = do
  ...
  where
    mkAttrs t =
      ...
      "target" =: "_blank" <>
      -- TODO(skylar): We can't use mkCardanoScanLink here cause Illegal type stuff
      "href" =: ("https://testnet.cardanoscan.io/transaction/" <>  t)

mkCardanoScanLink :: T.Text -> T.Text
mkCardanoScanLink = ("https://testnet.cardanoscan.io/transaction/" <>)
...
-}
spinner :: (PostBuild t m, DomBuilder t m) => m ()
spinner = do
  elDynAttrNS (Just "http://www.w3.org/2000/svg") "svg" (pure attrs) $ do
    let circleAttrs =
    ...
    elAttr "circle" circleAttrs blank
    elAttr "path" pathAttrs blank
  where
    attrs =
      "class" =: "animate-spin -ml-1 mr-3 h-5 w-5 text-white"
      <> "xmlns" =: "http://www.w3.org/2000/svg"

currentTime :: (Prerender t m, PostBuild t m, DomBuilder t m) => m (Dynamic t UTCTime)
currentTime = do
...
mkDuration s c
  ...
  | diff < hour = tShow (floor $ diff/60) <> " minute(s)"
  | otherwise = tShow (floor $ diff/hour) <> " hour(s)"
  where
...
currentBlock :: (MonadJSM (Performable m), MonadFix m, MonadIO (Performable m), TriggerEvent t m, PerformEvent t m, MonadHold t m, PostBuild t m) => m (Dynamic t Block)
currentBlock = do
  pb <- getPostBuild
...
pollTx :: (MonadJSM (Performable m), MonadFix m, MonadIO (Performable m), TriggerEvent t m, PerformEvent t m, MonadHold t m, PostBuild t m) => Dynamic t Nami.TxHash -> m (Dynamic t (Maybe Tx))
pollTx txHash = do
  pb <- getPostBuild
